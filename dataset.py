...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, csv_file, tokenizer):
        ...
        self.answers = self.df['target'].tolist()
    ...
    def __bleu_score(self, prediction_tokens, ground_truth_tokens):
        """_summary_
        ...
        Returns:
            _type_: _description_
        """
        # bleu_score_1gram  is number of 1-gram matches between prediction and ground truth
        bleu_score_1gram = sum(1 for word in prediction_tokens if word in ground_truth_tokens)
        return (bleu_score_1gram)
    ...
    def evaluate(self, predictions, gold_answers):
        ...
        for ground_truths, prediction in tqdm(zip(gold_answers, predictions)):
            ...
            bleu_score = self.__bleu_score(prediction, ground_truths)
        return 100*f1/len(predictions), 100*exact_match/len(predictions), 100*bleu_score/len(predictions)
